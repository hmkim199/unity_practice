...
public class PlatformSpawner : MonoBehaviour {
    ...
    void Start() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            platforms[i] = Instantiate(platformPrefab, poolPosition, Quaternion.identity); // Quaternion.identity는 오일러각 (0, 0, 0) 회전임.
        }
    ...
    void Update() {
        ...
        if (Time.time >= lastSpawnTime + timeBetSpawn) { // Time.time은 게임 시작 후 시간이 몇 초 지났는지 출력하는 유니티 라이버러리에 내장된 변수. 
